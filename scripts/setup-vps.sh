...
if [ $? -eq 0 ]; then
    ...
    echo "   Thử lại: docker compose --env-file .env.production -f docker-compose.prod.yml run --rm certbot certonly --webroot --webroot-path=/var/www/certbot --email ${EMAIL} --agree-tos --no-eff-email -d ${DOMAIN}"
fi
...
(crontab -l 2>/dev/null; echo "0 3 * * 1 cd ${APP_DIR} && docker compose --env-file .env.production -f docker-compose.prod.yml run --rm certbot renew && docker compose --env-file .env.production -f docker-compose.prod.yml restart nginx") | crontab -
